...
for (d in DEPTH){
  for( e in 1:length(ETA) ){
    forecasts <- data.frame(matrix(NA,nrow=nrow(data),ncol=2))
    ...
    for (t in 1:length(oos.seq)){
      ...
      ytrain <- data[data$date < oos.seq[t] , ]$RET
      ...
      if (t == length(oos.seq)){
        ...
        xgb.fc <- predict(xgb, xtest)
        forecasts$xgb[data$date >= oos.seq[t]] <- xgb.fc
        
      } else {
        
        xtest <- as.matrix(data[data$date >= oos.seq[t] & data$date < oos.seq[t+1] , which.x ])
        xgb.fc <- predict(xgb, xtest)
        forecasts$xgb[data$date >= oos.seq[t]  & data$date < oos.seq[t+1]] <- xgb.fc
    ...
    saveRDS(forecasts,sprintf('../forecasts/XGB_RETS_D%s_E%s_N100.RDS', d , ETA[e]))
  } 
  gc()
